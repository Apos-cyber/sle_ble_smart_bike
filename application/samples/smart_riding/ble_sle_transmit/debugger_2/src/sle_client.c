...
#define SLE_SEEK_INTERVAL_DEFAULT   200
#define SLE_SEEK_WINDOW_DEFAULT     200
#define UUID_16BIT_LEN 2
#define UUID_128BIT_LEN 16
#define SLE_HUNDRED   100        /* 100  */
#define DEFAULT_CONN_INTERVAL 0x14
#define DEFAULT_TIMEOUT_MULTIPLIER 0x1f4
#define DEFAULT_SCAN_INTERVAL 400
#define DEFAULT_SCAN_WINDOW 20
...
bool connect_book[12];//设置的是一次同时呼叫最多十二台设备同时连
...
{
    if (status == 0) {
        osal_printk("mac:%02x,%02x,%02x,%02x,%02x,%02x\r\n",g_send_mac[connect_num][0],g_send_mac[connect_num][1],g_send_mac[connect_num][2],g_send_mac[connect_num][3],g_send_mac[connect_num][4],g_send_mac[connect_num][5]);
        sle_addr_t addr;
        addr.type=0;
        (void)memcpy_s(&addr.addr, 6, g_send_mac[connect_num], 6);
        (void)memcpy_s(&g_remote_addr, sizeof(sle_addr_t), &addr, sizeof(sle_addr_t));
...
void sle_sample_seek_result_info_cbk(sle_seek_result_info_t *seek_result_data)
{
    if (seek_result_data != NULL) {
        for(uint8_t i=0;i<mac_num;i++)
        {
            if(connect_book[i]==0)
            {
                if (memcmp(seek_result_data->addr.addr, g_send_mac[i], SLE_ADDR_LEN) == 0) {
                    connect_book[i]=1;
...
{
    return (uint32_t)(((uint64_t)(in * SLE_HUNDRED)) / SLE_HUNDRED);
}
...
static void sle_notification_cb(uint8_t client_id, uint16_t conn_id, ssapc_handle_value_t *data,
    errcode_t status)
{
    unused(client_id);
    unused(status);
...
static void sle_indication_cb(uint8_t client_id, uint16_t conn_id, ssapc_handle_value_t *data,
    errcode_t status)
{
    unused(status);
    unused(conn_id);
    unused(client_id);
    osal_printk("\n sle_indication_cb sle uart recived data : %s\r\n", data->data);
}
    ...
    sle_acb_state_t conn_state, sle_pair_state_t pair_state, sle_disc_reason_t disc_reason)
{
    ...
    if (conn_state == SLE_ACB_STATE_CONNECTED) {
        sle_start_seek();//完成了一次连接后，开始连接下次搜索到的广播
    }
}
...
void sle_sample_pair_complete_cbk(uint16_t conn_id, const sle_addr_t *addr, errcode_t status)
{
    ...
    if (status == 0) {
        ...
        // info.version = 1;
        // ssapc_exchange_info_req(1, g_conn_id, &info);//把ssapc_exchange_info_req放到了sle_sample_connect_state_changed_cbk
    }
}
    ...
    errcode_t status)
{
    ...
    osal_printk("ready to disconnect!!!!!!!!!!\r\n");
    if(sle_disconnect_remote_device(&g_remote_addr)!=ERRCODE_SUCC)//这里断开连接
    {
        osal_printk("disconnect failed!!!!!!!\r\n");
    }
    // sle_stop_seek();//断开连接后关闭扫描，不知道为什么它会自动开启，然后重连两次
}
...
void sle_connect_param_init(void)
{
    sle_default_connect_param_t param = {0};
    param.enable_filter_policy = 0;
    param.gt_negotiate = 0;
    param.initiate_phys = 1;
    param.max_interval = DEFAULT_CONN_INTERVAL;
    param.min_interval = DEFAULT_CONN_INTERVAL;
    param.scan_interval = DEFAULT_SCAN_INTERVAL;
    param.scan_window = DEFAULT_SCAN_WINDOW;
    param.timeout = DEFAULT_TIMEOUT_MULTIPLIER;
