...
#define BLE_ADV_SERVICE_UUID_VALUE 0x1815
...
uint8_t g_uart_local_name[MAX_NAME_LENGTH] = { 's','l','e','_','b', 'i','k', 'e','_','t','e','s','t'};
...
static uint8_t ble_set_adv_flag_data(uint8_t *set_adv_data_position, uint8_t max_len)
{
    ble_adv_flag adv_flags = {
        .length = BLE_ADV_FLAG_LEN - BLE_GENERAL_BYTE_1+1,
        .adv_data_type = 1,
        .flags = BLE_ADV_FLAG_DATA+1
    };
...
static uint8_t ble_set_adv_service_uuid_data(uint8_t *set_adv_data_position, uint8_t max_len)
{
    ble_appearance_t adv_appearance_data = {
        .length = BLE_ADV_APPEARANCE_LENGTH - BLE_GENERAL_BYTE_1,
        .adv_data_type = 0x03,
        .catogory_id = { u16_low_u8(BLE_ADV_SERVICE_UUID_VALUE), u16_high_u8(BLE_ADV_SERVICE_UUID_VALUE) }
    };
...
static uint8_t ble_set_adv_appearance_data(uint8_t *set_adv_data_position, uint8_t max_len)
{
    ...
    idx += ble_set_adv_service_uuid_data(set_adv_data_position + idx, (max_len - idx));
...
uint8_t ble_uart_set_adv_data(void)
{
    ...
    for(uint16_t i = 0; i < adv_data_len; i++) {
        osal_printk("%02x ", set_adv_data[i]);
    }
    osal_printk("\n");
