...
static void process_adv_data(sle_seek_result_info_t *seek_result_data)
{
    uint8_t *adv_data = seek_result_data->data;
    uint8_t adv_data_len = seek_result_data->data_length;
    uint8_t index = 0;
    ...
    while (index < adv_data_len) {
        ...
        if (field_type == 0x09) { // Complete Local Name
            char local_name[SLE_BUFF_MAX_SIZE] = {0};
            uint8_t name_len = field_len - 1;
            memcpy_s(local_name, SLE_BUFF_MAX_SIZE, &adv_data[index + 2], name_len);
            osal_printk("%s local name: %s\r\n", SLE_UART_CLIENT_LOG, local_name);
        }
        index += field_len + 1;
    }
}
...
static void sle_uart_client_sample_seek_result_info_cbk(sle_seek_result_info_t *seek_result_data)
{
    if (seek_result_data == NULL || seek_result_data->data == NULL) {
        ...
        process_adv_data(seek_result_data);
        // sle_stop_seek();
    }
}
