...
void l610_mipcall_init(void)
{
    ...
    uint32_t stime = uapi_tcxo_get_ms();
    while (strstr((char *)l610_temp_buffer.buffer, "+SIM READY") == NULL) {
        uint32_t etime = uapi_tcxo_get_ms();
        if (etime - stime > 5000) {
            osal_printk("*******\r\nSIM card ready timeout.\r\n*******\r\n");
            hal_reboot_chip();
        }
        osal_msleep(100);
    } // 等待SIM卡准备好
    ...
    } while (l610_uart_send("AT+MIPCALL?\r\n", "+MIPCALL: 1") != BACK_EXPECT);
}
...
void l610_mqtt_init(void)
{
    ...
    l610_MQTTSub(test_sub_topic);// 等待订阅生效
...
{
    char test_pub_topic[] = "v1/devices/me/telemetry";
    char test_msg[100] = {0};
    double longitude, latitude;
    gnss_get_data(&longitude, &latitude);
    sprintf(test_msg, "{\\22longitude\\22:%.6f,\\22latitude\\22:%.6f}", longitude, latitude);
    l610_MQTTPub(test_pub_topic, test_msg);
}
